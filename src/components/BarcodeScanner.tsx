...
const BarcodeScanner: React.FC = () => {
    ...
    const [selectedResolution, setSelectedResolution] = useState<Resolution>({
        label: '1080p', // رزولوشن بالا برای دقت بهتر
        width: 1920,
        height: 1080,
    });
    ...
    const drawBarcodeOverlay = (positions: BarcodePosition[]) => {
        if (!canvasRef.current || !videoRef.current || videoRef.current.videoWidth === 0) return;
    ...
    const setupVideoStream = async (resolution: Resolution) => {
        setIsLoading(true);
        ...
        try {
            ...
            // منتظر آماده شدن متادیتای ویدئو
            ...
            const sendVideoFrame = () => {
                if (!ctx || !videoRef.current || !isMounted.current || videoRef.current.videoWidth === 0) return;
                ctx.drawImage(videoRef.current, 0, 0, canvas.width, canvas.height);
                const videoData = canvas.toDataURL('image/jpeg', 0.8);
                if (broadcastChannelRef.current) {
    ...
    useEffect(() => {
        const hints = new Map();
        hints.set(DecodeHintType.TRY_HARDER, true); // افزایش دقت برای بارکدهای چرخیده
        ...
        const scanMultiple = async () => {
            if (!videoRef.current || !isMounted.current || !readerRef.current || videoRef.current.videoWidth === 0) {
                // صبر کردن تا ویدئو آماده بشه
                setTimeout(scanMultiple, 100);
                return;
            }

            try {
                // اسکن مستقیم از ویدئو
                    ...
                    if (foundData) {
                        ...
                        if (snapCtx) {
                            ...
                            setSnapshots((prev) => {
                                ...
                                return [{ id: Date.now(), image: newSnapshot, barcode: code }, ...prev.slice(0, 9)];
                            });
                        }
                    }
            ...
            if (isMounted.current) {
                setTimeout(scanMultiple, 500); // فاصله برای کاهش بار پردازشی
            }
        };
